...
public class XiansPlatform
{
    ...
    private XiansPlatform(XiansOptions options, IHttpClientService? httpService, ITemporalClientService? temporalService)
    {
        ...
        var cacheLogger = Common.Infrastructure.LoggerFactory.CreateLogger<CacheService>();
        Cache = new CacheService(options.Cache, cacheLogger);
    ...
    public static async Task<XiansPlatform> InitializeForTestsAsync(
        string serverUrl = "http://no-server:0000", 
        string tenantId = "test-tenant", 
        string userId = "test-user",
        LogLevel? consoleLogLevel = LogLevel.Debug,
        LogLevel? serverLogLevel = null,
        bool enableTasks = false,
        Assembly[]? localModeAssemblies = null) {
        var options = new XiansOptions
        {
            ServerUrl = serverUrl,
            ApiKey = CertificateGenerator.GenerateTestCertificate(tenantId, userId),
            ConsoleLogLevel = consoleLogLevel,
            ServerLogLevel = serverLogLevel,
            LocalMode = true,
            EnableTasks = enableTasks,
            LocalModeAssemblies = localModeAssemblies
        };

        return await InitializeAsync(options);
    }
    ...
    public static async Task<XiansPlatform> InitializeAsync(XiansOptions options)
    {
        ...
        {
            var serverLogLevel = options.ServerLogLevel ?? Common.Infrastructure.LoggerFactory.GetServerLogLevel();
        ...
        ITemporalClientService? temporalService = null;
